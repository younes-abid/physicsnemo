...
from oil_tank_analyzer.core.config import PipelineConfig, PreprocessingConfig, DenoisingConfig, ContourConfig, CircleDetectionConfig
...
class PipelineGridSearch:
    ...
    def __init__(self):
        # Updated parameter grid with ALL our new methods including preprocessing
        self.param_grid = {
            'preprocessing': {
                'identity_preprocessor': {},
                'basic_normalization': {
                    'enhance_contrast': [True, False],
                    'clahe_clip_limit': [2.0, 3.0, 4.0],
                    'clahe_grid_size': [8, 16]
                },
                'tank_enhancement': {
                    'enhance_contrast': [True],
                    'enhance_circular': [True, False],
                    'suppress_background': [False, True],
                ...
                'multi_scale_enhancement': {
                    'scales': [[0.5, 1.0, 2.0], [0.75, 1.0, 1.5]],
                    'weights': [[0.2, 0.6, 0.2], [0.3, 0.4, 0.3]]
                }
            },
            'denoising': {
                'identity_denoiser': {
                ...
                'wavelet_sar': {
                    'level': [2, 3, 4],
                    'threshold_factor': [0.05, 0.1, 0.15],
                    'preserve_dynamic_range': [True]
                },
                'median_tank': {
                    'kernel_size': [3, 5, 7],
                ...
                'adaptive_tank': {
                    ...
                    'edge_threshold': [0.05, 0.1],
                    'preserve_dynamic_range': [True]
                },
                'multi_scale_tank': {
                    'scales': [[1.0, 0.5, 2.0], [0.75, 1.0, 1.5]],
                    'weights': [[0.6, 0.2, 0.2], [0.5, 0.3, 0.2]],
                    'preserve_dynamic_range': [True]
                },
                'gaussian': {
                    'kernel_size': [3, 5, 7],
            ...
            'edge_detection': {
                'xarray_contour_edges': {
                    'levels': [3, 5, 7],
                    'robust': [True, False],
                    'clean_edges': [True, False],
                    'enhance_contrast': [True, False],
                    'reduce_speckle': [True],
                    'remove_small_edges': [True],
                    'min_edge_length': [15, 20, 30],
                    'enhance_horizontal': [True, False]
                },
                'xarray_contourf_edges': {
                    'levels': [3, 5, 7],
                    'robust': [True, False],
                    'remove_outliers': [True, False]
                },
                'xarray_adaptive_contour': {
                    'percentiles': [[10, 30, 50, 70, 90], [20, 40, 60, 80]],
                    'robust': [True, False]
                },
                'xarray_multiscale_contour': {
                    'scales': [[1.0, 0.5, 2.0], [0.75, 1.0, 1.5]],
                    'levels': [3, 5, 7],
                    'robust': [True]
                },
                ...
                'gradient_magnitude_tank': {
                    'magnitude_threshold': [20, 30, 40]
                },
                'phase_congruency_tank': {
                    'pc_threshold': [20, 25, 30]
                },
                'morphological_tank': {
                    'kernel_size': [3, 5, 7],
                    'morph_threshold': [5, 10, 15]
                },
                'intensity_profile_tank': {
                    'profile_threshold': [1.0, 1.5, 2.0]
                }
            },
            'circle_detection': {
                'hough_tank': {
                ...
                'contour_tank': {
                    'min_area': [300, 500, 1000],
                    'max_area': [8000, 15000, 20000],
                    'min_circularity': [0.4, 0.5, 0.6],
                    'center_tolerance': [0.15, 0.2, 0.25],
                    'max_circles': [5, 8, -1]
                },
                'multi_scale_tank': {
        ...
        self.recommended_combinations = [
            ('tank_enhancement', 'enhanced_lee', 'xarray_contour_edges', 'multi_scale_tank'),
            ('tank_enhancement', 'lee_sigma', 'canny_tank', 'hough_tank'),
            ('basic_normalization', 'frost', 'adaptive_threshold_tank', 'ellipse_tank'),
            ('identity_preprocessor', 'median_tank', 'morphological_tank', 'contour_tank'),
            ('tank_enhancement', 'sar_bilateral', 'combined_tank', 'intensity_tank'),
            ('multi_scale_enhancement', 'adaptive_tank', 'xarray_adaptive_contour', 'template_tank'),
            ('tank_enhancement', 'nonlocal_means_sar', 'xarray_multiscale_contour', 'blob_detector')
        ]
    ...
    def _generate_exhaustive_configs(self) -> List[PipelineConfig]:
        ...
        for preprocess_method, preprocess_params in self.param_grid['preprocessing'].items():
            for denoise_method, denoise_params in self.param_grid['denoising'].items():
                for edge_method, edge_params in self.param_grid['edge_detection'].items():
                    for circle_method, circle_params in self.param_grid['circle_detection'].items():
                        ...
                        preprocess_param_combos = self._generate_param_combinations(preprocess_params)
                        ...
                        max_combos = 2
                        for pp in preprocess_param_combos[:max_combos]:
    ...
    def _generate_smart_configs(self) -> List[PipelineConfig]:
        ...
        for preprocess_method, denoise_method, edge_method, circle_method in self.recommended_combinations:
            preprocess_params = self.param_grid['preprocessing'].get(preprocess_method, {})
            ...
            for pp in self._sample_parameters(preprocess_params, 2):
        ...
        additional_configs = self._generate_random_configs(30)
    ...
    def _generate_random_configs(self, num_configs: int) -> List[PipelineConfig]:
        ...
        preprocess_methods = list(self.param_grid['preprocessing'].keys())
        ...
        for _ in range(num_configs):
            preprocess_method = np.random.choice(preprocess_methods)
            ...
            preprocess_params = self._sample_parameters(self.param_grid['preprocessing'][preprocess_method], 1)[0]
            ...
            config = PipelineConfig(
                preprocessing=PreprocessingConfig(
                    method=preprocess_method,
                    params=copy.deepcopy(preprocess_params)
                ),
                denoising=DenoisingConfig(
                    method=denoise_method, 
    ...
    def run_grid_search(self, image: np.ndarray, max_configs: int = 50, 
        ...
        if len(configs) > max_configs:
            ...
            for i, config in enumerate(configs):
                combo = (config.preprocessing.method, config.denoising.method, config.contour_detection.method, config.circle_detection.method)
        ...
        for i, config in enumerate(test_configs):
            print(f"Config {i+1}/{len(test_configs)}: "
                f"{config.preprocessing.method} + {config.denoising.method} + {config.contour_detection.method} + {config.circle_detection.method}")
